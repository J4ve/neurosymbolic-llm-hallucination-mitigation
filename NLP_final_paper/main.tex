...
The problem is especially damaging in mathematics. Puchalska and
Semadeni~\cite{puchalska1987} document that even grade-school students,
when shown arithmetic problems with missing, surplus, or contradictory
data, often produce a numeric answer rather than question the prompt; the
same failure mode persists in modern LLMs and is more consequential when
the downstream consumer is a financial system, a medical dosing tool, or
an automated grader. A student who omits a key value, or a dataset with
an entry error, can produce a problem that is mathematically unsolvable,
...
Section~\ref{sec:seeds} as few-shot examples in the user message. The deep contextual
...
problem. Figure~\ref{fig:hall} visualizes the gap.

\begin{figure}[t]
  \centering
  \includegraphics[width=\columnwidth]{fig_hallucination.pdf}
  \caption{Hallucination rate by system. Each bar shows the number of
  ill-defined problems (out of 400) for which the system produced a
  confident numeric answer rather than \texttt{REJECT}, expressed as a
  percentage.}
  \label{fig:hall}
\end{figure}
...
Table~\ref{tab:confusion} and Figure~\ref{fig:conf} show per-system
confusion matrices.

\begin{figure*}[t]
  \centering
  \includegraphics[width=0.9\textwidth]{fig_confusion.pdf}
  \caption{Confusion matrices as heatmaps (darker = higher count).
  The pipeline (right) concentrates predictions in the
  \texttt{Missing} column, including for solvable inputs, reflecting
  its over-abstention bias. Neither baseline ever predicts
  \texttt{Contra}.}
  \label{fig:conf}
\end{figure*}
...
and 198 for vanilla and CoT). The pipeline rejects 189 of the 200 solvable
inputs as Missing-type, meaning the 7B model's SMT-LIB translations leave
the target variable under-determined on roughly 94\% of solvable cases,
even with explicit prompt instructions to bound natural quantities by zero
and the auto-repair pass active. Z3 then satisfies the residual
constraints with multiple models, which our wrapper classifies as a
Missing-type rejection.

\textbf{Error analysis.} Two failure modes dominate pipeline errors.

\emph{Translation truncation.} For complex multi-step problems, the
greedy decoder occasionally truncates the SMT-LIB output before the
final asserts are written, even at our extended \texttt{max\_new\_tokens=768}
budget. As one example, on a GSM8K commission problem (``James sold 10
cars at \$25{,}000 each, paid 30\% to acquire them, takes 10\%
commission on profits; how much did he make?''), the translator
declares all relevant variables and writes the asserts for
\texttt{price\_per\_car}, \texttt{acquisition\_cost\_per\_car}, and
\texttt{profit\_per\_car}, but the output ends mid-assertion on
\texttt{(assert} before encoding the constraint linking
\texttt{total\_profit} and \texttt{total\_earnings} to those variables.
The remaining variables are then unconstrained, Z3 finds multiple
satisfying models, and the pipeline outputs \texttt{REJECT}.
...
The auto-repair pass (Section~\ref{sec:impl}) is not optional in our
setup. Without it, Z3's
...
multi-step word problems. The under-constraint rate of roughly 94\% on

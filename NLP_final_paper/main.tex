...
Large Language Models (LLMs) frequently produce confident yet incorrect answers
to mathematical problems that are missing key information or contain
...
Qwen2.5-7B-Instruct trails vanilla and chain-of-thought baselines on aggregate
macro F1 (0.210 vs.\ 0.474--0.482), because the 7B translator under-constrains
its SMT-LIB output and the pipeline over-abstains on solvable inputs. However,
the pipeline reduces the rate of confident wrong answers on ill-defined
problems from 19--21\% to 2\%, a roughly ten-fold reduction, and is the only
system that explicitly detects any contradictions. The results support the
project's hypothesis that separating language understanding from logical
evaluation eliminates a class of hallucinations unreachable by prompted LLMs,
while showing that closing the aggregate-F1 gap requires either a fine-tuned
translator or a larger base model.
...
\texttt{solvable}/\texttt{missing}/\texttt{contra} and \texttt{answer} is
an integer (for solvable rows) or \texttt{null} otherwise. Sampling uses a
...
\begin{table}[h]
  \centering
  \caption{Performance on the 600-problem balanced test set
  (200 solvable, 200 missing, 200 contra). \textbf{S}, \textbf{M},
  \textbf{C} are per-class F1. \textbf{AnsAcc} is answer accuracy among
  problems where both system and gold agree the problem is solvable; the
  parenthesized number is the support size.}
  \label{tab:main}
  \begin{tabular}{lcccccc}
    \toprule
    System & Acc & Macro F1 & S & M & C & AnsAcc \\
    \midrule
    Vanilla Qwen2.5-7B   & 0.587 & 0.474 & 0.798 & 0.623 & 0.000          & 0.761 (184) \\
    CoT Qwen2.5-7B       & \textbf{0.600} & \textbf{0.482} & \textbf{0.822} & \textbf{0.625} & 0.000          & \textbf{0.904 (198)} \\
    Ours (Few-shot + Z3) & 0.347 & 0.210 & 0.100 & 0.500 & \textbf{0.029} & 0.636 (11)  \\
    \bottomrule
  \end{tabular}
\end{table}
...
\begin{table}[h]
  ...
  \begin{tabular}{lcc}
    \toprule
    System & Hallucinated answers / 400 & Rate \\
    \midrule
...
\emph{Translation under-specification:} a problem like ``Larry has 3
cats, three times as many dogs as cats, \ldots, how many animals
total?'' produces a chain of definitions but no upper bound on derived
variables. Without an explicit upper bound or fully chained equality,
several models satisfy the constraints. \emph{Unit mismatches:} the
pipeline occasionally returns the right magnitude in the wrong unit
(e.g.~fraction 0.5 instead of percentage 50). SMT-LIB does not natively
carry unit information, so the pipeline cannot recover from these.
